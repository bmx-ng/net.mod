...
Extern
	...
	Function bmx_mbedtls_pk_parse_key:Int(handle:Byte Ptr, key:Byte Ptr, keylen:Int, pwd:Byte Ptr, pwdlen:Int, cb:Byte Ptr, rng:Object)
	Function bmx_mbedtls_pk_parse_key_string:Int(handle:Byte Ptr, key:String, pwd:String, cb:Byte Ptr, rng:Object)
...
Rem
bbdoc: Polling the net context failed.
End Rem
Const MBEDTLS_ERR_NET_POLL_FAILED:Int = -$0047
Rem
bbdoc: Input invalid.
End Rem
Const MBEDTLS_ERR_NET_BAD_INPUT_DATA:Int = -$0049
...
Rem
bbdoc: Internal-only message signaling that further message-processing should be done
end rem
Const MBEDTLS_ERR_SSL_CONTINUE_PROCESSING:Int = -$6580
Rem
bbdoc: The asynchronous operation is not completed yet.
end rem
Const MBEDTLS_ERR_SSL_ASYNC_IN_PROGRESS:Int = -$6500
Rem
bbdoc: Internal-only message signaling that a message arrived early.
end rem
Const MBEDTLS_ERR_SSL_EARLY_MESSAGE:Int = -$6480
Rem
bbdoc: An encrypted DTLS-frame with an unexpected CID was received.
end rem
Const MBEDTLS_ERR_SSL_UNEXPECTED_CID:Int = -$6000
Rem
bbdoc: An operation failed due to an unexpected version or configuration.
end rem
Const MBEDTLS_ERR_SSL_VERSION_MISMATCH:Int = -$5F00
Rem
bbdoc: Invalid value in SSL config */
end rem
Const MBEDTLS_ERR_SSL_BAD_CONFIG:Int = -$5E80
